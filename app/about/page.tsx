...
export default async function AboutPage() {
  const [profile, links] = await Promise.all([getProfile(), getSocialLinks()])
  return <main className="page-shell">
    <header><h1 className="display-title">{profile?.artistName ?? 'Artist Name'}</h1><p className="eyebrow">{profile?.subtitle}</p></header>
    {profile?.portraitPath && <figure className="relative mt-16 aspect-[4/5] w-full overflow-hidden bg-[#ddd]"><Image src={profile.portraitPath} alt={`${profile.artistName} in the studio`} fill priority unoptimized={profile.portraitPath.startsWith('/media/') || profile.portraitPath.startsWith('http')} sizes="(max-width: 840px) 100vw, 800px" className="object-cover" /></figure>}
    <div className="prose-copy mt-14">{profile?.bio && <Biography>{profile.bio}</Biography>}</div>
