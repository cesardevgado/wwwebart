...
export async function loginAction(_: ActionState, formData: FormData): Promise<ActionState> {
  const password = text(formData, 'password'); const user = await db.select().from(adminUsers).where(eq(adminUsers.id, 1)).get()
...
export async function saveProfileAction(_: ActionState, formData: FormData): Promise<ActionState> {
  ...
  let portraitPath = (await db.select().from(siteSettings).where(eq(siteSettings.id, 1)).get())?.portraitPath
  const uploadedPortraitUrl = text(formData, 'uploadedPortraitUrl')
  const portrait = formData.get('portrait')
  try { if (uploadedPortraitUrl) portraitPath = uploadedPortraitUrl; else if (portrait instanceof File && portrait.size) portraitPath = (await storage.upload(portrait, 'image')).publicUrl } catch (error) { return { error: (error as Error).message } }
  let links: unknown
  ...
  await db.transaction(async (tx) => {
    await tx.update(siteSettings).set({ artistName: parsed.data.artistName, subtitle: parsed.data.subtitle, bio: parsed.data.bio, portraitPath, updatedAt: new Date().toISOString() }).where(eq(siteSettings.id, 1)).run()
...
export async function saveArtworkAction(id: number | null, _: ActionState, formData: FormData): Promise<ActionState> {
  ...
  const duplicate = await db.select({ id: artworks.id }).from(artworks).where(id ? and(eq(artworks.slug, parsed.data.slug), ne(artworks.id, id)) : eq(artworks.slug, parsed.data.slug)).get()
  if (duplicate) return { error: 'That slug is already in use.' }
  let mediaPath = text(formData, 'uploadedMediaUrl') || parsed.data.mediaUrl || (id ? (await db.select().from(artworks).where(eq(artworks.id, id)).get())?.mediaPath : null)
  ...
  if (id) await db.update(artworks).set(record).where(eq(artworks.id, id)).run()
  else { const max = (await db.select().from(artworks).all()).length; await db.insert(artworks).values({ ...record, sortOrder: max, createdAt: new Date().toISOString() }).run() }
...
export async function deleteArtworkAction(id: number) {
  await requireAdmin(); const artwork = await db.select().from(artworks).where(eq(artworks.id, id)).get()
  if (artwork?.mediaPath && storage.owns(artwork.mediaPath)) await storage.delete(artwork.mediaPath)
  await db.delete(artworks).where(eq(artworks.id, id)).run(); revalidatePath('/'); redirect('/admin/artworks?deleted=1')
}
export async function reorderArtworksAction(ids: number[]) {
  ...
  await db.transaction(async (tx) => { for (const [sortOrder, id] of valid.entries()) await tx.update(artworks).set({ sortOrder, updatedAt: new Date().toISOString() }).where(eq(artworks.id, id)).run() })
...
export async function addArtworkMediaAction(artworkId: number, _: ActionState, formData: FormData): Promise<ActionState> {
  await requireAdmin()
  if (!await db.select({ id: artworks.id }).from(artworks).where(eq(artworks.id, artworkId)).get()) return { error: 'Artwork not found.' }
  ...
  const type = typeResult.data; let mediaPath = text(formData, 'uploadedMediaItemUrl') || text(formData, 'mediaItemUrl')
  ...
  const count = (await db.select().from(artworkMedia).where(eq(artworkMedia.artworkId, artworkId)).all()).length
  await db.insert(artworkMedia).values({ artworkId, type, path: mediaPath, altText: text(formData, 'mediaItemAlt') || null, sortOrder: count, autoplay: formData.has('mediaAutoplay'), muted: formData.has('mediaMuted'), loop: formData.has('mediaLoop'), controls: formData.has('mediaControls'), createdAt: new Date().toISOString() }).run()
...
export async function deleteArtworkMediaAction(id: number) {
  await requireAdmin(); const item = await db.select().from(artworkMedia).where(eq(artworkMedia.id, id)).get()
  if (item?.path && storage.owns(item.path)) await storage.delete(item.path)
  if (item) { await db.delete(artworkMedia).where(eq(artworkMedia.id, id)).run(); revalidatePath(`/admin/artworks/${item.artworkId}`); revalidatePath('/artwork/[slug]', 'page') }
}
export async function reorderArtworkMediaAction(artworkId: number, ids: number[]) {
  ...
  await db.transaction(async (tx) => { for (const [sortOrder, id] of valid.entries()) await tx.update(artworkMedia).set({ sortOrder }).where(and(eq(artworkMedia.id, id), eq(artworkMedia.artworkId, artworkId))).run() })
