...
export async function generateStaticParams() { return (await getPublishedArtworks()).map(({ slug }) => ({ slug })) }
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  const artwork = await getPublishedArtwork((await params).slug)
  if (!artwork) return {}
...
export default async function ArtworkPage({ params }: { params: Promise<{ slug: string }> }) {
  const artwork = await getPublishedArtwork((await params).slug)
  if (!artwork) notFound()
  const [{ previous, next }, profile, media] = await Promise.all([getPublishedNeighbors(artwork.slug), getProfile(), getArtworkMedia(artwork.id)])
  return <main className="page-shell !pt-5">
    ...
    <ArtworkViewer artwork={artwork} media={media} />
