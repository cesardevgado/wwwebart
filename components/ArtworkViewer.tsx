...
export function ArtworkViewer({ artwork, media = [] }: { artwork: Artwork; media?: MediaItem[] }) {
  ...
  return <div className="space-y-5">{ordered.map((item,index) => {
    if (item.type === 'image') return <div key={item.id} className={frame}><Image src={item.path} alt={item.altText || `${artwork.title}, image ${index + 1}`} fill priority={index === 0} sizes="(max-width: 840px) 100vw, 800px" unoptimized={item.path.startsWith('/media/')} className="object-contain" /></div>
