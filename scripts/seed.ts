...
if ((await db.select().from(artworks).all()).length === 0) await db.insert(artworks).values([
...
const liminalField = await db.select().from(artworks).where(eq(artworks.slug, 'liminal-field')).get()
if (liminalField && (await db.select().from(artworkMedia).where(eq(artworkMedia.artworkId, liminalField.id)).all()).length === 0) {
  await db.insert(artworkMedia).values([
